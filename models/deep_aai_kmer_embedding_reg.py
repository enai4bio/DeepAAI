...
class DeepAAIKmerEmbeddingReg(nn.Module):
    def __init__(self, **param_dict):
        super(DeepAAIKmerEmbeddingReg, self).__init__()
        ...
        self.global_linear = nn.Linear(self.h_dim, self.h_dim)
    ...
    def forward(self, **ft_dict):
        ...
        :return:
        '''
        device = ft_dict['antibody_graph_node_ft'].device
        antibody_res_mat = torch.zeros(ft_dict['antibody_graph_node_ft'].size()[0], self.h_dim).to(device)
        virus_res_mat = torch.zeros(ft_dict['virus_graph_node_ft'].size()[0], self.h_dim).to(device)

        antibody_node_ft = self.antibody_kmer_linear(ft_dict['antibody_graph_node_ft'])
        ...
        # virus
        virus_node_ft = self.virus_kmer_linear(ft_dict['virus_graph_node_ft'])
        ...
        # cross
        global_pair_ft = virus_res_mat + antibody_res_mat + (virus_res_mat * antibody_res_mat) * self.cross_scale
        ...
        pair_ft = global_pair_ft + local_pair_ft
